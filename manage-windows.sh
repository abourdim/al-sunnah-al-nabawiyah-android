#!/usr/bin/env bash
# ============================================================================
#  Al Sunnah al-Nabawiyah Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/16-al-sunnah-al-nabawiyah}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.alsunnahalnabawiyah}"
AVD_NAME="${AVD_NAME:-al_sunnah_al_nabawiyah_test}"
...
cmd_test() {
  ...
  local out="/tmp/al-sunnah-al-nabawiyah-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Al Sunnah al-Nabawiyah Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
