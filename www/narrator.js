...
(function() {
  ...
  function cleanText(text) {
    ...
    if (l === 'fr') {
      clean = clean
        ...
        .replace(/al-Sunnah/gi, 'al Sounna')
    ...
    if (l === 'en') {
      clean = clean
        ...
        .replace(/al-Sunnah/gi, 'al-Soonnah')
  ...
  function readDuoTranslation(card, onEnd) {
    ...
    // Try anxiety data
    if (!duoText && card.type === 'anxiety' && typeof REFLECTIONS !== 'undefined') {
  ...
  function setupMediaSession() {
    ...
    navigator.mediaSession.metadata = new MediaMetadata({
      title: l === 'ar' ? 'السنة النبوية' : l === 'fr' ? 'La Sunna Prophétique' : 'Prophetic Sunnah',
  ...
  function onSpeedChange(val) {
    ...
    localStorage.setItem('sn-narrator-speed', STATE.speed);
  }
  ...
  function onPitchChange(val) {
    ...
    localStorage.setItem('sn-narrator-pitch', STATE.pitch);
  }
  ...
  function onKaraokeToggle(checked) {
    ...
    localStorage.setItem('sn-narrator-karaoke', checked);
  }

  function onAutoScrollToggle(checked) {
    STATE.autoScroll = checked;
    localStorage.setItem('sn-narrator-autoscroll', checked);
  }

  function onDuoToggle(checked) {
    STATE.duoReading = checked;
    localStorage.setItem('sn-narrator-duo', checked);
  }
  ...
  function loadSettings() {
    STATE.speed = parseFloat(localStorage.getItem('sn-narrator-speed')) || 1;
    STATE.pitch = parseFloat(localStorage.getItem('sn-narrator-pitch')) || 1;
    STATE.karaokeEnabled = localStorage.getItem('sn-narrator-karaoke') !== 'false';
    STATE.autoScroll = localStorage.getItem('sn-narrator-autoscroll') !== 'false';
    STATE.duoReading = localStorage.getItem('sn-narrator-duo') === 'true';
  }
  ...
  var origRenderReflections = window.renderReflections;
  if (origRenderReflections) {
    window.renderReflections = function() { origRenderReflections(); setTimeout(injectSpeakButtons, 100); };
  }
